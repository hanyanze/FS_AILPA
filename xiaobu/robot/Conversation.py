...
from robot import logging, ASR, TTS, AI, Player, config, constants, utils
...
class Conversation(object):
    ...
    def pardon(self):
        if not self.hasPardon:
            self.say("刚刚没听清，能再说一遍吗？", onCompleted=lambda: self.doResponse(self.activeListen()))
            self.hasPardon = True
        else:
            # self.say("没听清呢")
            self.hasPardon = False
    ...
    def activeListen(self, silent=False):
        ...
        try:
            if not silent:
                #time.sleep(1)
                Player.play(constants.getData(random.choice(self._response_wav)))
            listener = snowboydecoder.ActiveListener([constants.getHotwordModel(config.get('hotword', 'xiaobu.pmdl'))])
